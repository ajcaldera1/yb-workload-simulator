...
import com.yugabyte.simulation.service.WorkloadParameterResolver;
...
public class WorkloadSimulationController {
    ...
    @Value("${spring.workload:genericWorkload}")
    private String defaultWorkloadBean;
    ...
    private class WorkloadRunner implements Runnable {
        ...
        public void run() {
            ...
            if (workload.getInvoker() != null) {
                ...
                workloadExecutionService.execute(defaultWorkloadBean, workload.getWorkloadId(),
                        workloadParameterResolver.toParameterMap(workload, paramsToUse));
            }

            try {
                do {
                    Thread.sleep(1000);
                } while (workloadManager.getActiveWorkloads().size() > 1);
            } catch (Exception e) {
                e.printStackTrace();
            } finally {
                exit(0);
    ...
    private void setWorkloadName() {
        this.workloadSimulation = workloadRegistryService.getWorkload(defaultWorkloadBean);
    ...
    public InvocationResult invokeWorkload(@PathVariable String workload, @RequestBody ParamValue[] params) {
        try {
            WorkloadDesc operation = workloadRegistryService.getOperation(defaultWorkloadBean, workload);
            Map<String, Object> paramMap = workloadParameterResolver.toParameterMap(operation, params);
            ExecutionResult result = workloadExecutionService.execute(defaultWorkloadBean, workload, paramMap);
            if (result.getResult() != 0) {
                return new InvocationResult(new Exception(result.getMessage()));
            }
    ...
    @ResponseBody
    public Map<String, WorkloadResult> getResults(@PathVariable(name = "afterTime") long afterTime) {
